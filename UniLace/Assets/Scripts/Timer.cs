...
public class Timer
{
    ...
    public virtual RosMessageTypes.BuiltinInterfaces.TimeMsg Now()
    {
        Now(out uint secs, out uint nsecs);
        return new RosMessageTypes.BuiltinInterfaces.TimeMsg(secs, nsecs);
    }

    public virtual void Now(RosMessageTypes.BuiltinInterfaces.TimeMsg stamp)
    {
        uint secs; uint nsecs;
        Now(out secs, out nsecs);
        stamp.sec = secs; stamp.nanosec = nsecs;
    }
